...
def check_profile(
    ws, row
):  # compare number H and length of profile kol. A wit kol B - grey
    ...
    for row in range(min_row, max_row + 1):  # find right component
        ...
        if (
            ...
            if "+" not in str(right_quantity):  # not yet merged
                quantity = (
                    str(right_quantity).strip() + "+" + str(left_quantity).strip() + "L"
                )
