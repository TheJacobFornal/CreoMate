...
def find_first_empty_row_purchuses(ws):
    for row in range(8, ws.max_row + 2):
        if (
            not ws.cell(row, 2).value is None
            or not ws.cell(row, 3).value is None
            or not ws.cell(row, 4).value is None
            or not ws.cell(row, 6).value is None
            or not ws.cell(row, 7).value is None
            or not ws.cell(row, 8).value is None
        ):
            continue
        else:
            return row
